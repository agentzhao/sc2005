...
void Lock::Acquire() {
  ...
  lock_owner = currentThread; // the locked has to be owned by the current
                              // thread for the released to happen
  isLock = 1;
...
bool Lock::isHeldByCurrentThread() { return (lock_owner == currentThread); }
#endif
...
Condition::~Condition() { delete Conqueue; }

// waiting for input or smth, releases lock and goes to sleep
