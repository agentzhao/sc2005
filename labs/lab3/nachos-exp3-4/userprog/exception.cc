  ...
  if (!kernelArgs) {
    ...
    if ((s = strtok(kernelArgs, " "))) {
      ...
      while ((s = strtok(0, " "))) {
        ...
        len = strlen(s) + 1;
        len +=
            (4 - (len % 4)) % 4; // next multiple of 4, if not a multiple of 4
        bytesToAllocate += len;
      }
    }
...
int SyscallRead(char *buffer, int size, OpenFileId id) {
  ...
  else if ((numBytes = FileRead(kernelBuffer, size, id)) == -1) // Read error
    return -1;
...
char *UserToKernelString(char *source) {
  ...
  while (*temp2++ =
             machine->mainMemory[GetPhysAddrInKernel((int)source++, FALSE)])
    ;
...
void ReadConsoleAvail(int arg) { synchConsole->readAvail->V(); }
...
void WriteConsoleDone(int arg) { synchConsole->writeDone->V(); }
