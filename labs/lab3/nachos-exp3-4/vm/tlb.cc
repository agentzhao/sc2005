...
int VpnToPhyPage(int vpn) {
  ...
  // you can refer to PageOutPageIn(int vpn) to see how an entry was created in
  // ipt returns index/frame number for this vpn, or -1 if vpn is not in IPT
  for (int i = 0; i < NumPhysPages; i++) {
    if (memoryTable[i].valid && memoryTable[i].vPage == vpn &&
...
void InsertToTLB(int vpn, int phyPage) {
  ...
  // your code to find an empty in TLB or to replace the oldest entry if TLB is
  // full
  i = FIFOPointer;
  ...
  if (memoryTable[phyPage].valid) { // check if pageOut possible
    ...
    if (memoryTable[phyPage].dirty) { // pageOut is necessary
      if ((mmapPtr = GetMmap(memoryTable[phyPage].vPage))) { // it's mmaped
        ...
        numBytesWritten = mmapPtr->openFile->WriteAt(
            machine->mainMemory + phyPage * PageSize, mmapBytesToWrite,
        ...
        DEBUG('p', "paging out: pid %i, phyPage %i, vpn %i\n",
              memoryTable[phyPage].pid, phyPage, memoryTable[phyPage].vPage);
        numBytesWritten = memoryTable[phyPage].swapPtr->WriteAt(
            machine->mainMemory + phyPage * PageSize, PageSize,
...
void DoPageIn(int vpn, int phyPage) {
  ...
  if ((mmapPtr = GetMmap(vpn))) { // mmaped file
    ...
    numBytesRead = mmapPtr->openFile->ReadAt(
        machine->mainMemory + phyPage * PageSize, mmapBytesToRead,
        (vpn - mmapPtr->beginPage) * PageSize);
    ASSERT(numBytesRead == mmapBytesToRead);
  } else { // not mmaped
    DEBUG('p', "paging in: pid %i, phyPage %i, vpn %i\n", currentThread->pid,
          phyPage, vpn);
    numBytesRead = currentThread->space->swapPtr->ReadAt(
        machine->mainMemory + phyPage * PageSize, PageSize, vpn * PageSize);
...
int lruAlgorithm(void) {
  ...
  for (int i = 0; i < NumPhysPages; i++) {
    ...
    if (memoryTable[i].lastUsed < earliest) {
      earliest = memoryTable[i].lastUsed;
      phyPage = i;
    }
  }
  return phyPage;
